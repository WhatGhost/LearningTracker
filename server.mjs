...
async function handleApi(request, response, url) {
  if (request.method === "GET" && url.pathname === "/api/articles") {
    ...
    sendJson(response, 200, listArticles({ search: url.searchParams.get("search") || "", status }));
    return true;
  }
  ...
  if (request.method === "GET" && url.pathname === "/api/export") {
    ...
    sendJson(
      response,
      200,
      { version: 1, exportedAt: new Date().toISOString(), articles: exportArticles() },
...
export const server = createServer(async (request, response) => {
  ...
  try {
    ...
    const statusCode = error?.code === "DUPLICATE_URL"
      ? 409
      : message.includes("不能为空") || message.includes("无效") || message.includes("过大") || message.includes("HTTP")
        ? 400
        : 500;

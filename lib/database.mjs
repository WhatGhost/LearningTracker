...
const DEFAULT_NETWORK_SETTINGS = Object.freeze({
  ...
  httpProxy: process.env.LEARNING_TRACKER_HTTP_PROXY || "http://127.0.0.1:17890",
  socksProxy: process.env.LEARNING_TRACKER_SOCKS_PROXY || "socks5://127.0.0.1:10801",
...
export function importArticles(items) {
  ...
  try {
    for (const item of items) {
      ...
      if (Number(result.changes) === 1) insertedIds.push(Number(result.lastInsertRowid));
      else duplicates += 1;
    }
    database.exec("COMMIT");
  } catch (error) {

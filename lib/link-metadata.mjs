...
function markdownCandidates(line) {
  const candidates = [];
  const pattern = /\[([^\[\]\r\n]*)\]\(\s*(https?:\/\/[^\s)]+)\s*\)/giu;
...
function removeHomepageCandidates(candidates) {
  return candidates.filter((candidate) => {
    const current = new URL(candidate.url);
    const isHomepage = current.pathname === "/" && !current.search;
    if (!isHomepage) return true;
    return !candidates.some((other) => {
      if (other === candidate) return false;
      const compared = new URL(other.url);
      return compared.hostname === current.hostname && (compared.pathname !== "/" || Boolean(compared.search));
    });
  });
}
...
export function parseLinks(input) {
  ...
  for (const line of String(input).split(/\r?\n/u)) {
    const rawCandidates = markdownCandidates(line);
    const candidates = [];

    for (const candidate of rawCandidates.length ? rawCandidates : bareCandidates(line)) {
      try {
        candidates.push({ ...candidate, url: normalizeUrl(trimUrlPunctuation(candidate.rawUrl)) });
      } catch {
        // Skip malformed URL candidates while preserving the valid lines.
      }
    }

    for (const candidate of removeHomepageCandidates(candidates)) {
      if (seen.has(candidate.url)) continue;
      seen.add(candidate.url);
      results.push({ url: candidate.url, suppliedTitle: "" });
...
function readMetaDescription(html) {
  ...
  for (const tag of metaTags) {
    ...
    const content = tag.match(/content\s*=\s*["']([^"']+)["']/iu)?.[1];
    if (content) return decodeEntities(content).slice(0, 2_000);
  }
  return "";
}
...
export function metadataHttpErrorMessage(status, hostname = "") {
  const normalizedHostname = String(hostname).toLowerCase();
  const isZhihu = normalizedHostname === "zhihu.com" || normalizedHostname.endsWith(".zhihu.com");
  if (status === 403 && isZhihu) {
    return "知乎返回了访问验证（HTTP 403），当前抓取器无法执行验证脚本。请手动填写标题并选择标签后导入。";
  }
  return `网页返回 ${status}`;
}
...
async function fetchHtml(startUrl) {
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    ...
    if (!response.ok) throw new Error(metadataHttpErrorMessage(response.status, parsedUrl.hostname));
